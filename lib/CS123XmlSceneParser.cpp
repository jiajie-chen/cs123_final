...
#define PARSE_ERROR(e) cout << ERROR_AT(e) << "could not parse <" << e.tagName().toStdString() \
    << ">" << endl
#define UNSUPPORTED_ELEMENT(e) cout << ERROR_AT(e) << "unsupported element <" \
    << e.tagName().toStdString() << ">" << endl;
...
bool CS123XmlSceneParser::parse() 
{
    ...
    {
        cout << "parse error at line " << errorLine << " col " << errorColumn << ": "
             << errorMessage.toStdString() << endl;
        return false;
    }
    file.close();
...
{
    GLfloat *valuePtr = glm::value_ptr(m);
...
bool CS123XmlSceneParser::parseTransBlock(const QDomElement &transblock, CS123SceneNode* node)
{
    ...
    while (!childNode.isNull())
    {
        ...
        else if (e.tagName() == "rotate")
        {
            ...
            float angle;
            if (!parseQuadruple(
                e, t->rotate.x, t->rotate.y, t->rotate.z, angle, "x", "y", "z", "angle"))
            {
                PARSE_ERROR(e);
                return false;
            }
        ...
        else if (e.tagName() == "object")
        {
            if (e.attribute("type") == "master")
            {
                ...
                {
                    cout << ERROR_AT(e) << "invalid master object reference: " << masterName
                        << endl;
                    return false;
                }
